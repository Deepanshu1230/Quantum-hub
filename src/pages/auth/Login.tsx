...
import { useNavigate, Link } from "react-router-dom"; // Use useNavigate and Link from react-router-dom
import { Helmet, HelmetProvider } from "react-helmet-async"; // Reintroduce Helmet
...
export default function Login() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // Demo auth: flag the user as authed locally
      localStorage.setItem("qh_authed", "1");
      toast({ title: "Welcome back to QuantumHub" });
      navigate("/quantumhub", { replace: true }); // Use navigate
    } catch (e) {
  ...
  return (
    <HelmetProvider>
      <Helmet>
        <title>Log in | QuantumHub</title>
        <meta name="description" content="Log in to QuantumHub to access AI predictions, business optimization, and secure password generation." />
            ...
            <form onSubmit={onSubmit} className="space-y-6">
              <div className="space-y-2">
                ...
                <Input 
                  ...
                  required 
                  className="bg-gray-800/50 border border-gray-700 text-white placeholder:text-gray-500 focus:border-purple-500 focus:ring-purple-500 rounded-xl h-12 text-lg px-4"
                />
              </div>
              <div className="space-y-2">
              ...
              <p className="text-sm text-gray-400 text-center mt-4">
                New here?{" "}
                <Link to="/signup" className="underline text-purple-400 hover:text-purple-300 transition-colors">
                  Create an account
                </Link>
              </p>
